...
class TestValidateQuery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_validate_query_blocks_obfuscated_ips(self):
        # Hex representation of 127.0.0.1
        with self.assertRaises(ValueError) as cm1:
            await bot_logic.validate_query("http://0x7f000001/admin")
        self.assertIn("restricted network", str(cm1.exception))

        # Octal representation of 127.0.0.1
        with self.assertRaises(ValueError) as cm2:
            await bot_logic.validate_query("http://0177.0.0.1/admin")
        self.assertIn("restricted network", str(cm2.exception))

        # Integer representation of 169.254.169.254
        with self.assertRaises(ValueError) as cm3:
            await bot_logic.validate_query("http://2852039166/latest/meta-data/")
        self.assertIn("restricted network", str(cm3.exception))

    async def test_validate_query_allows_external_url(self):
