...
class TestValidateQuery(unittest.IsolatedAsyncioTestCase):
    ...
    @unittest.mock.patch('asyncio.wait_for')
    @unittest.mock.patch('asyncio.get_running_loop')
    async def test_validate_query_allows_external_url(self, mock_get_loop, mock_wait_for):
        query = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"

        # Mock successful DNS resolution returning public IP
        mock_loop = MagicMock()
        mock_get_loop.return_value = mock_loop
        mock_wait_for.return_value = [(socket.AF_INET, socket.SOCK_STREAM, 6, '', ('8.8.8.8', 0))]
    ...
    @unittest.mock.patch('asyncio.wait_for')
    @unittest.mock.patch('asyncio.get_running_loop')
    async def test_validate_query_blocks_obfuscated_ips_loopback(self, mock_get_loop, mock_wait_for):
        query = "http://safe-looking-domain.com/"

        # Mock DNS resolution returning a loopback IP
        mock_loop = MagicMock()
        mock_get_loop.return_value = mock_loop
        mock_wait_for.return_value = [(socket.AF_INET, socket.SOCK_STREAM, 6, '', ('127.0.0.1', 0))]

        with self.assertRaises(ValueError) as cm:
            await bot_logic.validate_query(query)
        self.assertIn("is blocked for security reasons", str(cm.exception))

    @unittest.mock.patch('asyncio.wait_for')
    @unittest.mock.patch('asyncio.get_running_loop')
    async def test_validate_query_blocks_obfuscated_ips_private(self, mock_get_loop, mock_wait_for):
        query = "http://safe-looking-domain.com/"

        # Mock DNS resolution returning a private IP
        mock_loop = MagicMock()
        mock_get_loop.return_value = mock_loop
        mock_wait_for.return_value = [(socket.AF_INET, socket.SOCK_STREAM, 6, '', ('10.0.0.1', 0))]
