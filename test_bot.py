...
class TestValidateQuery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_validate_query_blocks_obfuscated_ip(self):
        query = "http://0x7f000001"
        with self.assertRaises(ValueError) as cm:
            await bot_logic.validate_query(query)
        self.assertIn("This host is blocked", str(cm.exception))
    ...
    async def test_validate_query_blocks_unresolvable_host(self):
        query = "http://this.domain.does.not.exist.at.all.com"
        with self.assertRaises(ValueError) as cm:
            await bot_logic.validate_query(query)
