...
class TestValidateQuery(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_validate_query_blocks_integer_ip(self):
        query = "http://2130706433" # 127.0.0.1
        with self.assertRaises(ValueError) as cm:
            await bot_logic.validate_query(query)
        self.assertIn("This host is blocked", str(cm.exception))

    async def test_validate_query_blocks_hex_ip(self):
        query = "http://0x7f.0x00.0x00.0x01" # 127.0.0.1
        with self.assertRaises(ValueError) as cm:
            await bot_logic.validate_query(query)
        self.assertIn("This host is blocked", str(cm.exception))

    async def test_validate_query_blocks_local_dns(self):
        query = "http://localtest.me" # resolves to 127.0.0.1
        with self.assertRaises(ValueError) as cm:
            await bot_logic.validate_query(query)
        self.assertIn("This host is blocked", str(cm.exception))
